...
import { getDefaultHubIdAction } from "@/lib/services/hub";

/**
 * Hook to sync hub ID to URL
 * - If URL has hub param: do nothing (middleware handles cookie sync)
...
export function useHubSync() {
  ...
  useEffect(() => {
    const syncHub = async () => {
      // If URL already has hub param, middleware will sync it to cookie
      if (urlHubId) {
        return;
      }
